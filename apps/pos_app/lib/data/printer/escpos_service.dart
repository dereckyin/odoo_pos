...
class EscPosReceiptBuilder {
  EscPosReceiptBuilder({
    this.paperWidth = PaperSize.mm80,
    this.charset = EscPosCharset.big5,
  });
  final PaperSize paperWidth;
  final EscPosCharset charset;
  ...
  Future<List<int>> build(
    ...
    final bytes = <int>[];
    bytes.addAll(escBeginJob(charset));

    bytes.addAll(await escText(gen, storeName,
        styles: const PosStyles(align: PosAlign.center, height: PosTextSize.size2, width: PosTextSize.size2)));
    bytes.addAll(gen.feed(1));
    bytes.addAll(await escText(gen, '=' * 32, styles: const PosStyles(align: PosAlign.center)));
    ...
    bytes.addAll(await escRow2(gen, left: '單號', right: orderRef));
    if (tableLabel != null && tableLabel.isNotEmpty) {
      bytes.addAll(await escRow2(gen, left: '桌號', right: tableLabel));
    }
    bytes.addAll(await escRow2(gen, left: '時間', right: df.format(order.createdAt.toLocal())));
    bytes.addAll(await escRow2(gen, left: '收銀', right: order.cashierId.substring(0, 8)));
    bytes.addAll(await escText(gen, '-' * 32, styles: const PosStyles(align: PosAlign.center)));

    for (final line in order.lines) {
      final optionLabel = line.selectedOptions.displayLabel;
      bytes.addAll(await escText(
        gen,
        optionLabel.isEmpty ? line.productName : '${line.productName} / $optionLabel',
      ));
      if (line.note != null && line.note!.isNotEmpty) {
        bytes.addAll(await escText(gen, '  備註: ${line.note}'));
      }
      bytes.addAll(await escRow2(
        gen,
        left: '  ${line.qty} x ${_money(line.unitPrice)}',
        right: _money(line.lineTotal),
      ));
    }
    bytes.addAll(await escText(gen, '-' * 32, styles: const PosStyles(align: PosAlign.center)));

    bytes.addAll(await _kv(gen, '小計', order.subtotal));
    if (!order.discount.isZero) {
      bytes.addAll(await _kv(gen, '折扣', order.discount.negate));
    }
    bytes.addAll(await _kv(gen, '稅(內含) 5%', order.tax));
    bytes.addAll(await escRow2(
      gen,
      left: '合計',
      right: _money(order.total),
      leftStyles: const PosStyles(bold: true, height: PosTextSize.size2),
      rightStyles: const PosStyles(align: PosAlign.right, bold: true, height: PosTextSize.size2),
    ));

    bytes.addAll(gen.feed(1));
    for (final pay in order.payments) {
      bytes.addAll(await escRow2(gen, left: pay.method.label, right: _money(pay.amount)));
    }

    if (invoice != null && invoice.invoiceNumber != null) {
      bytes.addAll(gen.feed(1));
      bytes.addAll(await escText(gen, '-' * 32, styles: const PosStyles(align: PosAlign.center)));
      bytes.addAll(await escText(gen, '發票號碼: ${invoice.invoiceNumber}',
          styles: const PosStyles(align: PosAlign.center, bold: true)));
      if (invoice.invoiceDate != null) {
        bytes.addAll(await escText(gen, '發票日期: ${df.format(invoice.invoiceDate!)}',
            styles: const PosStyles(align: PosAlign.center)));
      }
      if (invoice.carrier?.code != null) {
        bytes.addAll(await escText(gen, '載具: ${invoice.carrier!.code}',
            styles: const PosStyles(align: PosAlign.center)));
      }
    }

    bytes.addAll(gen.feed(2));
    bytes.addAll(await escText(gen, '謝謝惠顧 歡迎再來', styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(gen.feed(2));
    bytes.addAll(gen.cut());
    return bytes;
  }

  Future<List<int>> _kv(Generator gen, String k, Money v) =>
      escRow2(gen, left: k, right: _money(v));
  ...
  Future<List<int>> build(KitchenTicket ticket) async {
    ...
    for (final ln in ticket.lines) {
      ...
      if (ln.note != null && ln.note!.isNotEmpty) {
        bytes.addAll(await escText(gen, '  → ${ln.note}'));
      }
      bytes.addAll(gen.feed(1));
    }
...
class EscPosConfirmationBuilder {
  ...
  Future<List<int>> build(OrderConfirmation order, {String storeName = '點餐趣'}) async {
    ...
    bytes.addAll(await escText(gen, '餐點確認單（非發票）',
        styles: const PosStyles(align: PosAlign.center, bold: true)));
    bytes.addAll(await escText(gen, '=' * 32, styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(await escRow2(
      gen,
      left: '桌號',
      right: order.tableLabel,
      rightStyles: const PosStyles(align: PosAlign.right, bold: true),
    ));
    if (order.orderRef != null) {
      bytes.addAll(await escRow2(gen, left: '單號', right: order.orderRef!));
    }
    bytes.addAll(await escRow2(gen, left: '時間', right: df.format(order.placedAt.toLocal())));
    bytes.addAll(await escText(gen, '-' * 32, styles: const PosStyles(align: PosAlign.center)));
    ...
    for (final ln in order.lines) {
      final label = ln.optionsLabel != null && ln.optionsLabel!.isNotEmpty
          ? '${ln.name} / ${ln.optionsLabel}'
          : ln.name;
      bytes.addAll(await escText(gen, label));
      if (ln.note != null && ln.note!.isNotEmpty) {
        bytes.addAll(await escText(gen, '  備註: ${ln.note}'));
      }
      bytes.addAll(await escRow2(
        gen,
        left: '  ${ln.qty} x',
        right: ln.lineTotal.format(withSymbol: true),
      ));
    }
    ...
    bytes.addAll(await escRow2(
      gen,
      left: '預估合計',
      right: order.estimatedTotal.format(withSymbol: true),
      leftStyles: const PosStyles(bold: true),
      rightStyles: const PosStyles(align: PosAlign.right, bold: true),
    ));
    if (order.note != null && order.note!.isNotEmpty) {
      bytes.addAll(gen.feed(1));
      bytes.addAll(await escText(gen, '備註: ${order.note}', styles: const PosStyles(bold: true)));
    }
    bytes.addAll(gen.feed(2));
    bytes.addAll(await escText(gen, '請核對品項，結帳時以櫃台金額為準',
        styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(gen.feed(2));
    bytes.addAll(gen.cut());
    return bytes;
...
class EscPosInvoiceBuilder {
  ...
  Future<List<int>> build({
    ...
    bytes.addAll(await escText(gen, '電子發票證明聯',
        styles: const PosStyles(align: PosAlign.center, bold: true, height: PosTextSize.size2)));
    bytes.addAll(await escText(gen, storeName, styles: const PosStyles(align: PosAlign.center)));
    if (storeTaxId != null && storeTaxId.isNotEmpty) {
      bytes.addAll(await escText(gen, '賣方 $storeTaxId', styles: const PosStyles(align: PosAlign.center)));
    }
    bytes.addAll(gen.feed(1));
    bytes.addAll(await escText(
      gen,
    ...
    bytes.addAll(await escText(gen, invoiceNumber,
        styles: const PosStyles(align: PosAlign.center, bold: true, height: PosTextSize.size2)));
    bytes.addAll(await escText(gen, df.format(local), styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(await escText(gen, '隨機碼 $randomCode', styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(await escText(gen, '總計 ${total.format(withSymbol: true)}',
        styles: const PosStyles(align: PosAlign.center, bold: true)));
    if (buyerTaxId != null && buyerTaxId.isNotEmpty) {
      bytes.addAll(await escText(gen, '買方 $buyerTaxId', styles: const PosStyles(align: PosAlign.center)));
    }
    bytes.addAll(gen.feed(1));
...
class EscPosTableQrBuilder {
  ...
  Future<List<int>> build({
    ...
    bytes.addAll(await escText(gen, storeName,
        styles: const PosStyles(align: PosAlign.center, height: PosTextSize.size2)));
    bytes.addAll(await escText(gen, '桌號 $tableLabel',
        styles: const PosStyles(align: PosAlign.center, bold: true, height: PosTextSize.size2)));
    bytes.addAll(gen.feed(1));
    bytes.addAll(await escText(gen, '掃描 QR 點餐', styles: const PosStyles(align: PosAlign.center)));
    bytes.addAll(gen.qrcode(orderUrl, align: PosAlign.center));
    bytes.addAll(gen.feed(1));
    bytes.addAll(await escText(gen, '請勿分享此 QR', styles: const PosStyles(align: PosAlign.center)));
    if (expiresAt != null) {
      bytes.addAll(await escText(gen, '有效至 ${df.format(expiresAt.toLocal())}',
          styles: const PosStyles(align: PosAlign.center)));
    }
    bytes.addAll(gen.feed(2));
    bytes.addAll(gen.cut());
