...
對外瀏覽器僅能透過 **`https://pos.myvnc.com/api/...`**（由 Nginx 轉發）；注意 FastAPI 的 `/health` 在**根路徑**，未掛在 `/api` 下，因此 **`/api/health` 可能不成立**；對外健康檢查可改為檢查首頁或只從容器內測 `/health`。

## 5. Caddy（HTTPS）反代到 9088

參考 **`deploy/pos.myvnc.com.caddyfile.example`**，將 `pos.myvnc.com { ... }` 合併進主機 Caddy 設定後 reload。

確認瀏覽器開啟 **`https://pos.myvnc.com`** 能載入登入頁，且 Network 中 `POST .../api/auth/admin-login` 回應為 JSON（非空 body 的 403）以利除錯。
...
- 後台列印 QR 用的顧客網址：在 build `apps/admin` 前於 `.env.production` 或指令前綴設定，例如：  
  `VITE_CUSTOMER_BASE_URL=https://pos.myvnc.com/customer`（若已掛顧客靜態）。
